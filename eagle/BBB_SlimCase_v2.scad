...
module Base()
{
    ...
    // 3x temp sensors
    translate([BB_TEMP_X-Cmp_Gap+AdditionalSDCard_Space, BB_TEMP1_Y, Case_PCB_Z+BB_TEMP_Z]) {
        rotate([0,90,0.0]) {
           cylinder(h=10, d=BB_TEMP_D, $fn=25);
        }
    }
    translate([BB_TEMP_X-Cmp_Gap+AdditionalSDCard_Space, BB_TEMP2_Y, Case_PCB_Z+BB_TEMP_Z]) {
        rotate([0,90,0.0]) {
           cylinder(h=10, d=BB_TEMP_D, $fn=25);
        }
    }
    translate([BB_TEMP_X-Cmp_Gap+AdditionalSDCard_Space, BB_TEMP3_Y, Case_PCB_Z+BB_TEMP_Z]) {
        rotate([0,90,0.0]) {
           cylinder(h=10, d=BB_TEMP_D, $fn=25);
        }
    }
    ...
    // servo connector
    translate([BB_SERVO_X-Cmp_Gap+AdditionalSDCard_Space, BB_SERVO_Y-Cmp_Gap, Case_PCB_Z+BB_SERVO_Z]) cube([10+Cmp_Gap2+Case_Thickness+0.5, BB_SERVO_W+Cmp_Gap2, BB_SERVO_H+Cmp_Gap2]);
    
    // fan output
    translate([BB_FAN_X+Cmp_Gap, BB_FAN_Y, Case_PCB_Z+BB_FAN_Z]) {
        rotate([-90,0,0.0]) {
            cylinder(h=5, d=BB_TEMP_D, $fn=25);
        }
    }
		...
		//SD Card
    if (SDCARD_Port == 1) {
    translate([BB_SD_X-Cmp_Gap+AdditionalSDCard_Space, BB_SD_Y-Cmp_Gap, Case_PCB_Z-BB_SD_Z]) cube([BB_SD_L+Cmp_Gap2+Case_Thickness+0.5, BB_SD_W+Cmp_Gap2, BB_SD_Z+2*Cmp_Gap2]);
    }
		//HDMI
    if (HDMI_Port == 1) {
...
module MountingPeg()
{
    difference() {
        ...
        translate([0,0,0]) cylinder(h=Case_PCB_Z+BB_Z+0.1,d=BB_Hole_Screw, $fn=25);
    }
}
